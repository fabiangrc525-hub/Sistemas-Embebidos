...
class OV7670:
    def __init__(
        ...
        # Reset completo de la cámara antes de configurar
        self.i2c.writeto(i2c_id, bytearray([0x12, 0x80]))
        time.sleep(0.5)
    ...
    def capture(self, buf: bytearray):
        ...
        FIX GIL: el busy-wait `while self.dma.active(): pass` retenía el
        GIL de MicroPython, bloqueando el otro core (scheduler + TCP).
        Ahora cede el GIL con sleep_us(100) en cada iteración, permitiendo
        que core 0 ejecute el scheduler mientras el DMA transfiere.
        """
        self.dma.config(
            read=self.sm,
            write=buf,
        ...
        self.sm.active(1)
        self.dma.active(1)
        # Ceder el GIL mientras el DMA trabaja (no busy-wait puro)
